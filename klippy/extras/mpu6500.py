...
import logging, time, collections, threading, multiprocessing, struct, os
...
SCALE = (FREEFALL_ACCEL * 16)/(1<<15)

Accel_Measurement = collections.namedtuple(
    'Accel_Measurement', ('time', 'accel_x', 'accel_y', 'accel_z'))

# Helper class to obtain measurements
class MPUQueryHelper:
...
class MPUCommandHelper:
    def __init__(self, config, chip):
        ...
        if self.name == "mpu6500":
            self.register_commands(None)
    def register_commands(self, name):
        # Register commands
        gcode = self.printer.lookup_object('gcode')
        gcode.register_mux_command("XACCELEROMETER_MEASURE", "CHIP", name,
                                   self.cmd_ACCELEROMETER_MEASURE,
                                   desc=self.cmd_ACCELEROMETER_MEASURE_help)
        gcode.register_mux_command("XACCELEROMETER_QUERY", "CHIP", name,
                                   self.cmd_ACCELEROMETER_QUERY,
                                   desc=self.cmd_ACCELEROMETER_QUERY_help)
        gcode.register_mux_command("XACCELEROMETER_DEBUG_READ", "CHIP", name,
                                   self.cmd_ACCELEROMETER_DEBUG_READ,
                                   desc=self.cmd_ACCELEROMETER_DEBUG_READ_help)
        gcode.register_mux_command("XACCELEROMETER_DEBUG_WRITE", "CHIP", name,
    ...
    def cmd_ACCELEROMETER_MEASURE(self, gcmd):
        if self.bg_client is None:
            ...
            gcmd.respond_info("mpu measurements started")
            return
        # End measurements
        name = gcmd.get("NAME", time.strftime("%Y%m%d_%H%M%S"))
        if not name.replace('-', '').replace('_', '').isalnum():
            raise gcmd.error("Invalid mpu NAME parameter")
        bg_client = self.bg_client
        self.bg_client = None
        bg_client.finish_measurements()
        # Write data to file
        if self.name == "mpu":
            filename = "/tmp/mpu-%s.csv" % (name,)
        else:
            filename = "/tmp/mpu-%s-%s.csv" % (self.name, name,)
    ...
    def cmd_ACCELEROMETER_QUERY(self, gcmd):
        ...
        if not values:
            raise gcmd.error("No mpu measurements found")
        _, accel_x, accel_y, accel_z = values[-1]
        gcmd.respond_info("mpu values (x, y, z): %.6f, %.6f, %.6f"
...
# Printer class that controls MPU-6500 chip
class MPU6500:
    def __init__(self, config):
        self.printer = config.get_printer()
        MPUCommandHelper(config, self)
        self.query_rate = 0
        am = {'x': (0, SCALE), 'y': (1, SCALE), 'z': (2, SCALE),
              '-x': (0, -SCALE), '-y': (1, -SCALE), '-z': (2, -SCALE)}
        axes_map = config.getlist('axes_map', ('x','y','z'), count=3)
        if any([a not in am for a in axes_map]):
            raise config.error("Invalid mpu axes_map parameter")
        ...
        self.query_mpu6500_cmd = self.query_mpu6500_end_cmd = None
        self.query_mpu6500_status_cmd = None
        mcu.add_config_cmd("config_mpu6500 oid=%d spi_oid=%d"
                           % (oid, self.spi.get_oid()))
        mcu.add_config_cmd("query_mpu6500 oid=%d clock=0 rest_ticks=0"
                           % (oid,), on_restart=True)
        mcu.register_config_callback(self._build_config)
        mcu.register_response(self._handle_mpu6500_data, "mpu6500_data", oid)
        # Clock tracking
        ...
        wh.register_mux_endpoint("mpu6500/dump_mpu6500", "sensor", self.name,
    ...
    def set_reg(self, reg, val, minclock=0):
        ...
        if stored_val != val:
            raise self.printer.command_error(
                    "Failed to set MPU register [0x%x] to 0x%x: got 0x%x. "
                    "This is generally indicative of connection problems "
                    "(e.g. faulty wiring) or a faulty mpu chip." % (
                        reg, val, stored_val))
    # Measurement collection
    def is_measuring(self):
        return self.query_rate > 0
    def _handle_mpu6500_data(self, params):
        with self.lock:
    ...
    def _extract_samples(self, raw_samples):
        ...
        count = seq = 0
        samples = [None] * (len(0) * SAMPLES_PER_BLOCK)
        for params in raw_samples:
            ...
            for i in range(len(d) // BYTES_PER_SAMPLE):
                ...
                #      - ((yzhigh & 0x40) << 7))
                raw_xyz = stuct.unpack(">hhh", d_xyz)
    ...
    def _update_clock(self, minclock=0):
        ...
        for retry in range(5):
            params = self.query_mpu6500_status_cmd.send([self.oid],
                                                        minclock=minclock)
            fifo = params['fifo'] & 0x7f
            if fifo <= 32:
                break
        else:
            raise self.printer.command_error("Unable to query mpu fifo")
    ...
    def _start_measurements(self):
        ...
        dev_id = self.read_reg(REG_WHOAMI)
        if dev_id != MPU6500_DEV_ID:
            raise self.printer.command_error(
        ...
        self.set_reg(REG_USER_CTRL, 0x40) # enable SPI access to fifo
        self.set_reg(REG_ACCEL_CONFIG, ACC_SCALE_16G)
        self.set_reg(REG_ACCEL_CONFIG2, 0x08) # bypass the DLPF
        ...
        self.query_mpu6500_cmd.send([self.oid, reqclock, rest_ticks],
                                    reqclock=reqclock)
        logging.info("MPU6500 starting '%s' measurements", self.name)
    ...
    def _finish_measurements(self):
        if not self.is_measuring():
            return
        # Halt bulk reading
        params = self.query_mpu6500_end_cmd.send([self.oid, 0, 0])
        self.query_rate = 0
        with self.lock:
            self.raw_samples = []
        logging.info("MPU6500 finished '%s' measurements", self.name)
    # API interface
    ...
    def _handle_dump_mpu6500(self, web_request):
        self.api_dump.add_client(web_request)
        hdr = ('time', 'x_acceleration', 'y_acceleration', 'z_acceleration')
        web_request.send({'header': hdr})
    def start_internal_client(self):
        cconn = self.api_dump.add_internal_client()
        return MPUQueryHelper(self.printer, cconn)

def load_config(config):
    return MPU6500(config)
